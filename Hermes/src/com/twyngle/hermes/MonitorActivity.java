 ...
 * @author pchan
 */
public class MonitorActivity extends Activity implements IBeaconConsumer {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// IBeaconManager.getBeaconSimulator()).createTimedSimulatedBeacons();
	}
...
//	}
	...
	private void logToDisplay(final String line) {
		runOnUiThread(new Runnable() {
			public void run() {
				EditText editText = (EditText) MonitorActivity.this
	...
	public void onIBeaconServiceConnect() {
		iBeaconManager.setMonitorNotifier(new MonitorNotifier() {
			...
			public void didEnterRegion(Region region) {
				logToDisplay("I just saw an iBeacon named "
						+ region.getUniqueId() + " for the first time!");
			}

			@Override
			public void didExitRegion(Region region) {
				logToDisplay("I no longer see an iBeacon named "
						+ region.getUniqueId());
			}
		...
		try {
			...
			// Region("test2","DF7E1C79-43E9-44FF-886F-1D1F7DA6997B".toLowerCase(),
			// 1, 2));
			// iBeaconManager.startMonitoringBeaconsInRegion(new
			// Region("test3","DF7E1C79-43E9-44FF-886F-1D1F7DA6997C".toLowerCase(),
			// 1, 3));
			// iBeaconManager.startMonitoringBeaconsInRegion(new
			// Region("test4","DF7E1C79-43E9-44FF-886F-1D1F7DA6997D".toLowerCase(),
			// 1, 4));
